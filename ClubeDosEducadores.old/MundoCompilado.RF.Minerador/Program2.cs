...
namespace MundoCompilado.RF.Minerador
{
    class Program2
    {
        ...
        //static StreamReader _vagasTxt = new StreamReader("retificação.txt", System.Text.Encoding.GetEncoding(1252));
        ...
        static IEnumerable<VagaRemocao> _unidades;
        ...
        static void Main(string[] args)
        {
            ...
            _unidades = _vagaDomain.Get().Where(a => a.Data.Year == (DateTime.Now.Year + 1)).ToList();
        ...
        private static void ExecutarAnalise(bool processar = false)
        {
            if (!processar)
            {
                string linha;
        ...
        static void Processar(IEnumerable<VagaRemocao> vagas)
        {
            ...
            foreach (var v in vagas)
            {
                if(v.Id == 0)
                {
                    sb.AppendLine($@"INSERT INTO VagaRemocao (UnidadeId,CargoId,Vagas,[Data],VagasPotenciais,VagasIniciais) VALUES ({v.UnidadeId},{v.CargoId}, {v.Vagas}, CONVERT(date,getdate()),{v.VagasPotenciais}, {v.VagasIniciais});");
                    continue;
                }

                sb.AppendLine($@"update VagaRemocao set Vagas = {v.Vagas}, VagasPotenciais = {v.VagasPotenciais}, VagasIniciais = {v.VagasIniciais}, [Data] = CONVERT(date,getdate()) where id = {v.Id};");
            }
        ...
        static IEnumerable<VagaRemocao> TestarTxtFinal()
        {
            ...
            while ((valor = _vagasTxt.ReadLine()) != null)
            {
                ...
                if(vagaRemocao == null)
                {
                    vagaRemocao = new VagaRemocao { CargoId = cargo.Id, UnidadeId = unidadeId };
                }
